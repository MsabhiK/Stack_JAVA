...
/*Cindhuri ordered a coffee. Based on her order status, print the correct status message to the console. */
        ...
        ArrayList<Double> orderedTest = new ArrayList<Double>();
        orderedTest.add(cappuccino);
        orderedTest.add(latte);
...
Change the isReady flag value from true to false or vice versa in order to test your control logic (if-statement). */
        ...
        //OR
        System.out.println(isReadyOrder2 ? customer1 + " " + readyMessage : " " + pendingMessage);   // Conditionnelfunctionality of ternary operation
...
to test if all of your logic works, even when prices and statuses are changed. */
    ...
    /*public static void isReady(boolean readyStatus, String readyMessage, String displayTotalMessage, String pendingMessage, double totalPrices(ArrayList<Double> order)) {
        System.out.println(readyStatus ? customerName + readyMessage + " " +  displayTotalMessage + totalPrices(orderItem) : customerName + pendingMessage);
    }

    public static double totalPrices(ArrayList<Double> order) {
        double total=0;
